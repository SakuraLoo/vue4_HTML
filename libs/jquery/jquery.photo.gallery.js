	...
	photoGallery: function(options) {
		...
		$gallery.on("mouseover",function(e){
			...
			if(dragX > 0){
				...
				//var parentD = window.top.document, J = $(parentD.getElementById("J_pg"));
				var nextDragX=e.pageX || e.clientX;
				var nextDragY=e.pageY || e.clientY ;
				var o = $image.offset(),
				...
				if(w > cW){
					...
					left = o.left;
					//J.css('left', J.position().left + (nextDragX - dragX));
					J.css('left', 0);
				}
				if(h > cH){
					if(top > 0){
					...
					top = o.top;
					//J.css('top', J.position().top + (nextDragY - dragY));
					J.css('top',0);
				}
		...
		var isMax,preWidth, preHeight, preTop, preLeft;
  	 	...
  	 	$download.on("click", function(){
			...
			url = $image.attr("src");
			if(!url) return;
			var lineindex = url.lastIndexOf("\/");
		    imgName = url.substring(lineindex + 1, url.length);
			var pom = document.createElement('a');
	        pom.setAttribute('href', url);
	        pom.setAttribute('download', imgName);
...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		winWidth = Math.min(wW, imgWidth);
		winHeight = Math.min(wH, imgHeight);
		...
		localStorage["photoGalleryActiveIndex"] = 0;

		$("#J_pg").remove();
		/*$("<iframe></iframe").appendTo("body")
			.attr("id", "J_pg")
			.attr("src", "picview.html")
			.css({
				position : "absolute",
				left : (document.body.clientWidth - winWidth) /2,
				top : (document.body.clientHeight - winHeight) /2-80,
				width : winWidth,
				height : winHeight,
		...
		$("<iframe></iframe>").appendTo(window.parent.document.body).attr("id", "J_pg").attr("src", "./picview.html")
			.css({
				position : "absolute",
				left : 0,
				top : 0,
	...
	//当前页面范围内打开图片
	openPhotoGallerySelf : function(obj){
		...
		$("<iframe></iframe>").appendTo(document.body).attr("id", "J_pg").attr("src", "./picview.html")
			.css({
				position : "absolute",
				left : 0,
				top : 0,
	...
	initGallery : function(){
		...
		var dragX, dragY;
		var nextDragX, dragY;
		$(".dragWin").on("mouseenter",function(e){
			dragX = -1;
		}).on("mousedown",function(e){
			dragX=e.pageX || e.clientX;
			dragY=e.pageY || e.clientY;
		...
		$(".dragWin").remove(); // 功能缺陷, 只能慢慢拖动, 暂时注释掉
