var exam={
	...
	dragFn:function(){
		var _this=this;
		var data={},addname=0;
		$( "#ui_sortable_exam li" ).on('click', function (){
			addname++;
			data={
	...
	chooseFn:function(){
		...
		$(function (){
			addname++;
			data={
			    type:$("#ui_sortable_exam li").children('a').attr('data-checkType'),//1为单选，2为多选
			    //name命名规则，q代表前缀+父级li的题型id
			    name:'q'+$("#ui_sortable_exam li").attr('data-uid')+'_'+addname,
			    //生成1000以内的随机数
			    itmetid:addname+parseInt(1000*Math.random()),
				items:[{
						value:'0',
			...
			$('.ui-questions-content-list').append(template($("#ui_sortable_exam li").attr('data-tempId'),data));
			// _this.orderFn($('.ui-questions-content-list'));
		    // _this.sortFn();
		});
	}
